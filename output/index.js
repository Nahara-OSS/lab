...
// ../../../../AppData/Local/deno/npm/registry.npmjs.org/markdown/0.5.0/lib/markdown.js
var require_markdown = __commonJS({
  "../../../../AppData/Local/deno/npm/registry.npmjs.org/markdown/0.5.0/lib/markdown.js"(exports) {
    (function(expose) {
      ...
      expose.parse = function(source, dialect) {
        var md2 = new Markdown(dialect);
        return md2.toTree(source);
      };
      ...
      var mk_block = Markdown.mk_block = function(block, trail, line) {
        ...
        if (line != void 0) s.lineNumber = line;
        return s;
      };
      ...
      Markdown.prototype.toTree = function toTree(source, custom_root) {
        ...
        try {
          this.tree = custom_root || this.tree || [
            "markdown"
          ];
      ...
      Markdown.dialects.Gruber = {
        block: {
          ...
          code: function code(block, next) {
            var ret = [], re = /^(?: {0,3}\t| {4})(.*)\n?/, lines;
            if (!block.match(re)) return void 0;
            block_search: do {
          ...
          lists: function() {
            ...
            function regex_for_depth(depth) {
              return new RegExp(
                ...
                "(?:^(" + indent_re + "{0," + depth + "} {0,3})(" + any_list + ")\\s+)|(^" + indent_re + "{0," + (depth - 1) + "}[ ]{0,4})"
              );
            }
            ...
            return function(block, next) {
              ...
              if (!m) return void 0;
              function make_list(m2) {
                var list2 = bullet_list.exec(m2[2]) ? [
                  "bulletlist"
                ] : [
                  "numberlist"
                ];
              ...
              var stack = [], list = make_list(m), last_li, loose = false, ret = [
                stack[0].list
              ], i;
          ...
          blockquote: function blockquote(block, next) {
            if (!block.match(/^>/m)) return void 0;
            var jsonml = [];
            if (block[0] != ">") {
            ...
            var input = block.replace(/^> ?/gm, ""), old_tree = this.tree, processedBlock = this.toTree(input, [
              "blockquote"
          ...
          referenceDefn: function referenceDefn(block, next) {
            ...
            var b = this.loop_re_over_block(re, block, function(m) {
              ...
              if (m[4] !== void 0) ref.title = m[4];
              else if (m[5] !== void 0) ref.title = m[5];
            });
          ...
          para: function para(block, next) {
            return [
              [
                "para"
              ].concat(this.processInline(block))
            ];
          }
        }
      };
      Markdown.dialects.Gruber.inline = {
        ...
        "\\": function escaped(text) {
          if (this.dialect.inline.__escape__.exec(text)) return [
            2,
            text.charAt(1)
          ];
          else
        ...
        "[": function link(text) {
          ...
          var consumed = 1 + res[0], children = res[1], link2, attrs;
      ...
      function strong_em(tag, md2) {
        var state_slot = tag + "_state", other_slot = tag == "strong" ? "em_state" : "strong_state";
        function CloseTag(len) {
          this.len_after = len;
          this.name = "close_" + md2;
        }
        return function(text, orig_match) {
          if (this[state_slot][0] == md2) {
            this[state_slot].shift();
            return [
              text.length,
              new CloseTag(text.length - md2.length)
            ];
          } else {
            var other = this[other_slot].slice(), state = this[state_slot].slice();
            this[state_slot].unshift(md2);
            var res = this.processInline(text.substr(md2.length));
      ...
      function split_meta_hash(meta_string) {
        var meta = meta_string.split(""), parts = [
          ""
        ], in_quotes = false;
        while (meta.length) {
      ...
      Markdown.dialects.Maruku.block.document_meta = function document_meta(block, next) {
        if (block.lineNumber > 1) return void 0;
        if (!block.match(/^(?:\w+:.*\n)*\w+:.*$/)) return void 0;
        if (!extract_attr(this.tree)) {
          this.tree.splice(1, 0, {});
        }
        ...
        if (m = block.match(tight)) {
          ...
          for (var b = 0; b < blocks.length; ++b) {
            ...
            for (i = 0; i < defns.length; ++i) {
              list.push([
                "dd"
              ].concat(this.processInline(defns[i].replace(/(\n)\s+/, "$1"))));
            }
          }
        } else {
          return void 0;
      ...
      function extract_attr(jsonml) {
        return isArray(jsonml) && jsonml.length > 1 && typeof jsonml[1] === "object" && !isArray(jsonml[1]) ? jsonml[1] : void 0;
      }
      ...
      function render_tree(jsonml) {
        ...
        for (var a2 in attributes) {
          tag_attrs += " " + a2 + '="' + escapeHTML(attributes[a2]) + '"';
        }
...
// ../../../../AppData/Local/deno/npm/registry.npmjs.org/markdown/0.5.0/lib/index.js
var require_lib = __commonJS({
  "../../../../AppData/Local/deno/npm/registry.npmjs.org/markdown/0.5.0/lib/index.js"(exports) {
    exports.markdown = require_markdown();
    exports.parse = exports.markdown.toHTML;
  }
});

// viewer/index.ts
var import_markdown = __toESM(require_lib());
var index = (await fetch("./index.json")).json();
var listElement = document.querySelector("#list");
var view = document.querySelector("#view");
var readme = document.querySelector("#readme");
var seachbox = document.querySelector("#search");
async function search(query) {
  const resolvedIndex = (await index).filter((e) => query.length == 0 || e.name.includes(query) || e.tags.some((t) => t.includes(query)));
...
async function navigateTo(e) {
  ...
  readme.innerHTML = import_markdown.markdown.toHTML(markdown);
  selected = e.id;
}
search("");
